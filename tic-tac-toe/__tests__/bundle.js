...
function Board(height) {
	...
	var areAllEqual = function areAllEqual(arr) {
...
var Game = React.createClass({
	...
	move: function move(x, y, player) {
		...
		return new Promise(function (resolve, reject) {
			...
			if (winner) {
				self.setState({ winner: winner, freezeBoard: true });
				reject();
			} else {
				self = {};
				resolve();
	...
	playerMove: function playerMove(event) {
		var _event$target$dataset = event.target.dataset.cell.split('_');

		var _event$target$dataset2 = _slicedToArray(_event$target$dataset, 2);

		var x = _event$target$dataset2[0];
		var y = _event$target$dataset2[1];

		var isEmpty = this.board.getCell(x, y) === 0;

		if (isEmpty) {
			this.move(x, y, this.state.player).then(this.setState({ player: this.nextPlayer() })).catch(console.log('No winner yet!'));
		}
	},
	...
	render: function render() {
		...
		if (this.state.winner) {
			var msg = this.state.winner > 2 ? 'It is a tie' : 'Player ' + this.state.winner + ' wins!';
			announcement = React.createElement(
				'div',
		...
		var grid = this.board.board.map(function (row, rowIdx) {
			var cells = row.map(function (cell, cellIdx) {
				...
				return React.createElement('div', { className: classStr[cell], key: cellIdx,
...
},{"../board":159,"react":158}],161:[function(require,module,exports){
'use strict';
...
describe('Testing Game component', function () {
	...
	describe('test game props and state', function () {
		...
		beforeEach(function () {
			...
			component = ReactDOM.render(React.createElement(Game, { size: 3 }), node);
		});

		it('has to have correct props.size', function () {
			ReactDOM.render(React.createElement(Game, { size: 4 }), node);
		...
		it('has to throw error on incorrect props.size', function () {
			...
			var test = function test() {
				ReactDOM.render(React.createElement(Game, { size: 13 }), node);
			};
			expect(test).toThrow();
		});
	});
...
},{"../src/components/game":160,"react":158,"react-dom":2}]},{},[161]);
